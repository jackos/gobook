          ...
          case import_kernel_pb.ToClient.KindCase.OUTPUT:
            {
              const items = msg.getOutput().getContentList().map((c) => {
                ...
                return new vscode3.NotebookCellOutputItem(value, "text/html");
              });
...
function parseMarkdown(content) {
  ...
  function parseCodeBlock(leadingWhitespace, codeBlockStart) {
...
function activate(context) {
  ...
  context.subscriptions.push(vscode5.workspace.registerNotebookSerializer("markdown-notebook", new MarkdownProvider(), providerOptions));
}
function deactivate() {
  kernel.kill("SIGKILL");
}
